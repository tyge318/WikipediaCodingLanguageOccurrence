...
object WikipediaRanking {
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    langs.map(x => (x, occurrencesOfLang(x, rdd))).sortWith((e1, e2) => (e1._2).compareTo(e2._2)>0)
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap(x => langs.filter(lang => x.mentionsLanguage(lang)).map(lang => (lang, x))).groupByKey
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap(x => langs.filter(lang => x.mentionsLanguage(lang)).map(lang => (lang, 1))).reduceByKey(_ + _).sortBy(-_._2).collect().toList
  }
